...
const firstImg = Math.floor(Math.random() * recipes.length);
...
window.addEventListener('DOMContentLoaded', () => {
	let starRating = "";
	for (let i = 0; i < 5; i++) {
		if (i < recipes[firstImg].rating && (i + .5) != recipes[firstImg].rating) {
			starRating += `<span aria-hidden="true" class="icon-star">🌕</span>
`;
		}
		else if (i < recipes[firstImg].rating && (i + .5) == recipes[firstImg].rating) {
			starRating += `<span aria-hidden="true" class="icon-star">🌗</span>
`;
		}
		else {
			starRating += `<span aria-hidden="true" class="icon-star-empty">🌑</span>
`;
		}
	}
	let genreTags = "";
	recipes[firstImg].tags.forEach(tag => {
		genreTags += `<button class="genre">${tag}</button>
`
	});
	searchResults.insertAdjacentHTML("beforeend", `<div class="recepie">
            <img src="${recipes[firstImg].image}" alt="A photo of ${recipes[firstImg].name}">
            <div class="right-side">
			<div class="tags">
            	${genreTags}
			</div>
                <h2>${recipes[firstImg].name}</h2>
                <span class="rating" role="img" aria-label="Rating: ${recipes[firstImg].rating} out of 5 stars">
                    ${starRating}
                </span>
                <p class="desc">${recipes[firstImg].description}</p>
            </div>
        </div>`);
});
...
searchForm.addEventListener('submit', function(event) {
	...
	recipes.forEach(recipie => {
		if (recipie.name.toLowerCase().includes(searchBar.value.toLowerCase()) || recipie.description.toLowerCase().includes(searchBar.value.toLowerCase()) || recipie.tags.toString().toLowerCase().includes(searchBar.value.toLowerCase())) {
			searchedArray.push(recipie)
		}
	});
	console.log(searchedArray);
	sortedSearch = searchedArray.toSorted((a, b) => a.name.localeCompare(b.name));
	console.log(sortedSearch);
	sortedSearch.forEach(recipie => {
		let starRating = "";
	for (let i = 0; i < 5; i++) {
		if (i < recipie.rating && (i + .5) != recipie.rating) {
			starRating += `<span aria-hidden="true" class="icon-star">🌕</span>
`;
		}
		else if (i < recipie.rating && (i + .5) == recipie.rating) {
			starRating += `<span aria-hidden="true" class="icon-star">🌗</span>
`;
		}
		else {
...
`;
	...
	let genreTags = "";
	recipie.tags.forEach(tag => {
		genreTags += `<button class="genre">${tag}</button>
`
	});
	searchResults.insertAdjacentHTML("beforeend", `<div class="recepie">
            <img src="${recipie.image}" alt="A photo of ${recipie.name}">
            <div class="right-side">
			<div class="tags">
            	${genreTags}
			</div>
                <h2>${recipie.name}</h2>
                <span class="rating" role="img" aria-label="Rating: ${recipie.rating} out of 5 stars">
                    ${starRating}
                </span>
                <p class="desc">${recipie.description}</p>
            </div>
        </div>`);
	});
});